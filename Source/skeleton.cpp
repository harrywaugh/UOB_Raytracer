  ...
  float pitch = 0.0f;
  float yaw = 0.0f;
  vec4 light_position(0, -0.5, 0.0, 1.0);
  vec4 st_light_position(0, -1.1, -0.7, 1.0);
  ...
  void opencl_initialise(t_ocl *ocl)  {
    ...
    for (uint i = 0; i < triangles.size(); i++)  {
      ...
      triangle_colors[i]        = {triangles[i].color.x,  triangles[i].color.y,  triangles[i].color.z,  triangles[i].color.w};
    }

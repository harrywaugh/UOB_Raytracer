...
float3 direct_light(const Intersection intersection, local float3 *triangle_vertexes, local float3 *triangle_normals, float3 light_pos, int triangle_n, const float3 intersect_normal, const int global_id) {
  ...
  for (int i = 0; i < light_sources; i++)  {
    const float3 random_vec = (float3) (random(i*(global_id), light_spread), random(i*(global_id)*91.0f, light_spread), random(i*(global_id)*19.0f, light_spread));
    float3 ghost_dir = dir + random_vec;
    ...
    if (!in_shadow(start, ghost_dir, triangle_vertexes, triangle_normals, ghost_radius_sq, triangle_n)) {
      total_light += (light_color * max(dot(dir, intersect_normal+5*random_vec), 0.0f)) / ( 4 * ((float)M_PI) * radius_sq);
    }
  }
...
float3 secondary_light(float3 start, float3 dir, local float3 *triangle_vertexes, local float3 *triangle_normals, local float4 *triangle_colors, int triangle_n, float3 light_pos, const int global_id)  {
  ...
  if (closest_intersection2(start + bias*dir, dir, triangle_vertexes, triangle_normals, &intersect, triangle_n)) {
    ...
    light_accumulator *= triangle_colors[intersect.triangle_index].xyz;
    // printf("start (%f, %f, %f) intersect (%f, %f, %f)\n", start.x, start.y, start.z, intersect.position.x, intersect.position.y, intersect.position.z);
  }
...
{         /* accumulated magnitudes of velocity for each cell */
  ...
  for (float dy = y*rays_y; dy < (y+1)*rays_y; dy+=1)  {
    ...
    // Declare ray for given position on the screen. Rotate ray by current view angle
        float3 dir = (float3) (dx - (SCREEN_WIDTH*rays_x)/2.0f, dy - (SCREEN_HEIGHT*rays_y)/2.0f, focal_length);
        ...
        if (closest_intersection(camera_pos, dir, LOC_triangle_vertexes, LOC_triangle_normals, &intersect, triangle_n)) {
          ...
          if (color.w == 0.0f)  {
            ...
            final_color_total += color.xyz*(indirect_light+light); //Add indirect back!
